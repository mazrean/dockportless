...
Each project gets its own namespace, and SO_REUSEPORT allows both proxies to share port 1355.
...
curl http://web.frontend.localhost:1355/
curl http://storybook.frontend.localhost:1355/
curl http://api.backend.localhost:1355/
```

## How it works

1. Each `dockportless run` writes its mapping to `$XDG_RUNTIME_DIR/dockportless/`
2. Both proxies share port 1355 via SO_REUSEPORT
